...
class SearchSummonerByNameController {
  ...
  fun index(@RequestParam(value = "summonerName", required = true) summonerName: String) : String {
    ...
    val leagueEntries =
            LeagueEntry.getLeagueEntries(summoner.id, apikey)
    ...
    for (entry in leagueEntries) {
      val obj = JsonObject()
      obj.add("tier", gson.toJsonTree(entry.tier))
      obj.add("rank", gson.toJsonTree(entry.rank))
      ...
      var queueType :String = "Ranked Flex"
      if(entry.queueType == "RANKED_SOLO_5x5"){
        queueType = "Ranked Solo"
      }
      obj.add("queueType",gson.toJsonTree(queueType) )
      rankList.add(obj)
    }
    ...
    returnObj.add("leagueEntries", gson.toJsonTree(rankList))
